...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    def step(self, obs, state=None, mask=None, deterministic=True):
        if deterministic:
            ...
            #                                       {self.obs_ph: obs})
            action, value, neglogp, layers_list = self.sess.run(
                [self.deterministic_action, self.value_flat, self.neglogp, self.layers_list],
                {self.obs_ph: obs})
        else:
            #action, value, neglogp = self.sess.run([self.action, self.value_flat, self.neglogp],
            #                                       {self.obs_ph: obs})
            action, value, neglogp, layers_list = self.sess.run(
                [self.action, self.value_flat, self.neglogp, self.layers_list],
                {self.obs_ph: obs})
