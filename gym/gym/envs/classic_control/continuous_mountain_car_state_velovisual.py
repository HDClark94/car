...
class state_velovisual_Continuous_MountainCarEnv(gym.Env):
    metadata = {
    ...
    def step(self, action):
        ...
        # reward option one ()
        if ((position < (self.goal_position + (self.goal_width / 2))) and (
                position > (self.goal_position - (self.goal_width / 2))) and (np.round(velocity, decimals=2) == 0) and (
                self.rewarded == False)):
        ...
        self.obs[1] = self.state[1] + (action-1)*self.np_random.normal(0, self.obsError)  # velocity is same

        (action-1)*self.np_random.normal(0, self.obsError)
