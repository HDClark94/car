...
class MountainCarJinghuaOriginalEnv(gym.Env):
    metadata = {
    ...
    def step(self, position, action, exp):
        ...
        if position >= self.goal_position - 0.05 and position <= self.goal_position + 0.05:
            indicator = 1
        else:
            indicator = 0

        if indicator == 1 and velocity == 0 and exp == 0:
            done = 1
            reward = 10
        elif exp == 1 and position == 1:
            done = 2
            reward = 0
        else:
            done = 0
            reward = -0.005
        # done = bool(position >= self.goal_position -0.05 and position <= self.goal_position+0.05 and velocity == 0 and exp == 0)

        self.state = (indicator, velocity)
        return np.array(self.state), position, reward, done, {}
